...
class _BookingCalendarMainState extends State<BookingCalendarMain> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<BookingController>(
      builder: (_, controller, __) => Padding(
        ...
        child: (controller.isUploading) ? const BookingDialog() : bookingContent(controller),
      ),
    );
  }

  Column bookingContent(BookingController controller) {
    return Column(
      children: [
        CommonCard(
        ...
        widget.bookingExplanation ?? bookingExplanationDefault(),
        const SizedBox(height: 16),
  ...
  Widget bookingExplanationDefault() {
    return Row(
